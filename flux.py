    ...
    try:
        ...
        try:
            ...
            raise FileNotFoundError(
                f"Generated image not found: {remote_file_path}")

        print(
            f"Processing complete. Image available at /generated_images/{output_filename}")
        image_url = '/generated_images/' + output_filename

    except Exception as e:
        print(
            f"* generate_image / execute_Flux * Error generating image for prompt '{prompt}': {e}")
        image_url = None
...
def setup_prompt_gpt_request(section):
    ...
    request = {
        ...
        "messages": [
            {
                "role": "system",
                "content": ("You are a prompt engineer for image generation AI specializing in creating short, effective prompts."
                ...
                # "You are a content assistant who specializes in script analysis for short-form video production."
...
if __name__ == "__main__":
    a = generate_prompt(
        "푸바오는 최근 중국에서 태어난 자이언트 판다로, 많은 주목을 받고 있습니다. 중국의 여러 동물원에서 푸바오의 성장과 건강을 지속적으로 모니터링하고 있다고 합니다.")
    print(a)
