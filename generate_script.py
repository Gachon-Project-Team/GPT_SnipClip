...
def execute_script(news, query):
    ...
    for category, articles in news.items():
        url, header, request = setup_gpt_request(
            category, json.dumps(articles), query)
        # 반환결과 category, title, section, image 딕셔너리
...
def execute_gpt(url, header, request):
    ...
    if response.status_code == 200:
        ...
        print(
            f"* gen_script/execute_gpt * response Error: {response.status_code}")
        categories = None
...
def execute_image_map(section):
    ...
    # 반환결과 category, title, section, image 딕셔너리
    result = execute_gpt(url, header, request)
    return result
